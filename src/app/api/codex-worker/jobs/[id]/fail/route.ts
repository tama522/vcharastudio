...
import { failUserCodexWorkerJob } from "@/lib/app-repository";
...
export async function POST(
  ...
  try {
    const ipLimit = checkRequestRateLimit(request, "codex-worker-fail", {
      limit: 90,
      windowMs: 60 * 1000,
    });
    ...
    const workerLimit = checkRateLimit({
      key: `codex-worker-fail:${worker.id}`,
      limit: 45,
      windowMs: 60 * 1000,
    });
    ...
    const result = await failUserCodexWorkerJob(
      worker.userId,
      worker.id,
      id,
